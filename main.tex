...
Second, the size of the configuration set matters as much as its
content. Reconstructing the company's set from average mixes alone
yields very few distinct configurations when demand is stable --- at
low variability the horizon average, the thirds and the per-period
mixes all coincide --- and a set that small cannot serve the
programme. Crews do not work that way: they accumulate patterns by
trial, one for the days a single product runs heavy, another for when
two share the load. Reconstructing that variety (the average mix, one
pattern per dominant product, single-product patterns, each at several
fill levels) restores feasibility, and the number of configurations
required to reach it is itself informative: on the low-variability
instances the averaging rule supplies fewer than sixty, while serving
the programme takes around two hundred.

That is the practical cost of a fixed configuration set. It is not
that the procedure fails --- the plant plainly operates --- but that
keeping it feasible as the programme moves requires the planner to
maintain a large and growing catalogue by hand, and every configuration
in it was found by trial rather than by design.

...
\begin{longtable}[hbt!]{|p{0.14\textwidth}|p{0.82\textwidth}|}
    ...
    $\theta_{gl}$    & $\in\{0,1\}$. Selects which resource the utilisation floor is applied to: volume if $1$, payload if $0$.\\
...
{\color{blue}
Minimum truck utilisation. Full-truckload transport requires a
dispatched truck to be full, but a truck can be full in two different
ways: it may run out of space or it may run out of payload. Imposing
the requirement on volume alone is not a conservative simplification,
it is an infeasible one --- filling a fraction $\alpha$ of a
$97.9$~m$^{3}$ trailer with RTIs at $590$~kg/m$^{3}$ weighs $27.4$~t
against a $24$~t cap, so no truck is dispatchable at all and the
instance is empty. Summing the two ratios instead is the opposite
error: it accepts a truck that is $35\%$ full by volume and $20\%$ by
weight, which is full by neither. We therefore require a truck to
reach a fraction $\alpha$ of \emph{either} resource, using a binary
$\theta_{gl}$ to select which one binds:
\begin{align}
\sum_{i\in I}\sum_{h\in H}\sum_{r\in R(h)}
    \gamma_{gr}\,n_{hr}\,L_{gihlr}
&\;\ge\;\alpha\,\Gamma_K\bigl(Z_{gl}-1+\theta_{gl}\bigr)
\label{sb:eq:util}\\
\sum_{i\in I}\sum_{h\in H}\sum_{r\in R(h)}
    \omega_{gr}\,n_{hr}\,L_{gihlr}
&\;\ge\;\alpha\,pK^{w}\bigl(Z_{gl}-\theta_{gl}\bigr)
\qquad \forall g\in G,\; l\in L,
\label{sb:eq:utilw}
\end{align}
with $\omega_{gr}=w^{\mathrm{f}}_{r}$ in the supplier direction and
$\omega_{pr}=\min_{n\in M(r)}w_{n}$ in the plant direction, the
lightest product being the one that makes the requirement hardest to
meet. The same disjunction is imposed on each dispatched truck at
runtime, on the loads $V_{klnt}$ and $V^{\mathrm{e}}_{klrt}$ rather
than on the configuration's designed capacity, since a truck may be
loaded to less than the configuration allows.
}
...
{\color{blue}
\begin{align*}
&Z_{gl},\,\theta_{gl},\,X_{gihl},\,LX_{gihl},\,LY_{gihl},\,
  A_{gijl},\,B_{gijl},\,C_{gijl},\,D_{gijl},\,

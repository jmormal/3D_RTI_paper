...
no bound at all; Figure~\ref{fig:bound} states the rule exactly. The Lagrangian correction \eqref{eq:lagbound} is
...
figure for the incumbent heuristic. Figure~\ref{fig:pricing} shows the oracle and its two objectives. Three measures bring its cost down without changing what is solved. A node prices only the directions and
...
construction}, as Figure~\ref{fig:hull} shows on a real pool. For any weights, \eqref{eq:PPObjective} attains its
...
\end{enumerate}
Figure~\ref{fig:branching} summarises the rule. The two aggregate levels absorb the fractionality almost entirely: on
